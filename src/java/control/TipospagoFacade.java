...
public class TipospagoFacade extends AbstractFacade<Tipospago> {
    ...
    public List<Tipospago> consultarTipospago()
    {
        Query consulta= em.createNamedQuery("Tipospago.activos", Tipospago.class);
    ...
    {
        Query consulta= em.createNamedQuery("Tipospago.eliminados", Tipospago.class);
        List<Tipospago> lista = consulta.getResultList();
        if(!lista.isEmpty())
            return lista;
        else
