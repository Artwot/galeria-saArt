...
public class DetalleVentaFacade extends AbstractFacade<DetalleVenta> {
    ...
    {
        Query consulta= em.createNamedQuery("DetalleVenta.activos", DetalleVenta.class);
    ...
    public List<DetalleVenta> consultarEliminados()
    {
        Query consulta= em.createNamedQuery("DetalleVenta.eliminados", DetalleVenta.class);
        List<DetalleVenta> lista = consulta.getResultList();
        if(!lista.isEmpty())
            return lista;
        else
